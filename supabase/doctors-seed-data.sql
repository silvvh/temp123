-- Script para popular banco de dados com médicos de teste
-- Execute este script no SQL Editor do Supabase
--
-- IMPORTANTE: Este script usa usuários existentes em auth.users
-- Se você não tiver usuários, crie alguns primeiro através da interface do Supabase Auth
-- ou use a API de autenticação para criar contas de teste
...
-- Insert de médicos usando usuários existentes
DO $$
DECLARE
...
BEGIN
  ...
  IF user_count = 0 THEN
    RAISE EXCEPTION 'Nenhum usuário encontrado em auth.users. Por favor, crie pelo menos um usuário primeiro através da interface do Supabase Auth ou use a API de autenticação.';
  END IF;
